...
def _get_simulator(model_kwargs, metadata, acc_noise_std, vel_noise_std):
  ...
  simulator = learned_simulator.LearnedSimulator(
      num_dimensions=metadata['pos_dim'],
...
def serving_input_receiver_fn():
    ...
    """
    position = tf.placeholder(dtype=tf.float32, shape=[None, INPUT_SEQUENCE_LENGTH , 2], name='position')
    velocity = tf.placeholder(dtype=tf.float32, shape=[None, INPUT_SEQUENCE_LENGTH , 3], name='velocity') # change this !!!!!
    receiver_tensors = {'position': position,'velocity':velocity}
    features = {'position': position,'velocity':velocity}
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        rollout_estimator.export_saved_model('saved_model', serving_input_receiver_fn)
        predict_fn = predictor.from_saved_model(reload_latest_model())
        input_dict=get_one_step_input()
        for j in range(10):
            input_dict_temp = input_dict
            pred = predict_fn(input_dict)
            print(pred)