...
default_rollout_dir = 'learning_to_simulate/rollouts/Rope/rollout_train_0.pkl'

flags.DEFINE_string("rollout_path", default_rollout_dir, help="Path to rollout pickle file")
...
def main(unused_argv):
  ...
  def update(step_i):
    outputs = []
    t1 = plot_info[0][0].text(0, 0, 0.0, style ='italic',fontsize = 30, color ="green")
    t2 = plot_info[1][0].text(0, 0, 0.0, style ='italic',fontsize = 30, color ="green")
    text_list = [t1, t2]
    counter = 0
    for ax, trajectory, points in plot_info:
      for particle_type, line in points.items():
        ...
        loss = np.sum((plot_info[0][1][step_i, mask,0:2] - plot_info[1][1][step_i, mask,0:2])**2)
        text_list[counter].set_text(str(loss))
        outputs.append(line)
      counter += 1
