...
default_model_dit = 'learning_to_simulate/models/Rope'
...
flags.DEFINE_string('model_path', default_model_dit,
  ...
  def input_fn():
    ...
    if mode.startswith('one_step'):
      ...
      if mode == 'one_step_train':
        ds = ds.repeat()
        ds = ds.shuffle(100000) # important
...
def rollout(simulator, features, num_steps):
  ...
  ground_truth_positions = features['position'][:,start_index+INPUT_SEQUENCE_LENGTH:]
  ground_truth_velocities = features['velocity'][:,start_index+INPUT_SEQUENCE_LENGTH:]
  
  global_context = features.get('step_context')
  def step_fn(step, current_positions, current_controls, predictions):
    ...
    next_positions = tf.concat([current_positions[:, 1:],
                                next_position_ground_truth[:, tf.newaxis]], axis=1) # next_position
    # The trick is to not update control
    next_control = ground_truth_velocities[:, step]
    next_controls = tf.concat([current_controls[:, 1:],
                                next_control[:, tf.newaxis]], axis=1)
    # next_controls = tf.reshape(next_control, [-1, INPUT_SEQUENCE_LENGTH, DIM])
    return (step + 1, next_positions, next_controls, updated_predictions)

  predictions = tf.TensorArray(size=num_steps, dtype=tf.float32)
  _, _, _, predictions = tf.while_loop(
      cond=lambda step, state, control, prediction: tf.less(step, num_steps),
      body=step_fn,
      loop_vars=(0, initial_positions, initial_control, predictions),
      back_prop=False,
...
def get_one_step_estimator_fn(data_path,
  ...
  def estimator_fn(features, labels, mode):
    ...
    pred_target = simulator.get_predicted_and_target_normalized_accelerations(
        ...
        control_input_sequence=features['velocity'],
        n_particles_per_example=features['n_particles_per_example'],
        particle_types=features['particle_type'],
        global_context=features.get('step_context'))
    ...
    min_lr = 1e-6
    lr = tf.train.exponential_decay(learning_rate=5e-4 - min_lr,
