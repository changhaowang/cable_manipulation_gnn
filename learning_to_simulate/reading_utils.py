...
_FEATURE_DTYPES = {
    ...
    'velocity': {
        'in': np.float32, #32 # control input
        'out': tf.float32 #32
    },
    'step_context': {
        'in': np.float32, #32
...
_CONTEXT_FEATURES = {
    'key': tf.io.FixedLenFeature([], tf.int64, default_value=0), #int64
    'particle_type': tf.io.VarLenFeature(tf.string) #tf.string
}
...
def convert_to_tensor(x, encoded_dtype):
  if len(x) == 1:
    ...
    out = encoded_dtype(x[0].numpy())
  else:
    out = []
    for el in x:
      # out.append(np.frombuffer(el.numpy(), dtype=encoded_dtype))
      out.append(encoded_dtype(el.numpy()))
...
def split_trajectory(context, features, window_length=7):
  ...
  pos_stack = []
  for idx in range(input_trajectory_length):
    # print(features['position'][idx:idx + window_length])
